...
bool AVLTree::rotateRightLeft(TreeNode *currNode, TreeNode *parent, TreeNode *grandParentNode)
{
  ...
  updateAVLParentChild(currNode->right,parent,false);
  updateAVLParentChild(currNode->left,grandParentNode,true);
...
bool checkParentImbalance(TreeNode* node){
  if((node -> parent -> left -> height) - (node -> parent -> right -> height) > 1) return true;
  return false;
}
...
int AVLTree::find(int jrnyCode)
{
  ...
  // see fit.
    ...
    while (ptr != nullptr)
    {
       if (ptr -> getJourneyCode() == find_node -> getJourneyCode()) //  equal case
       {
        return true;
       }
